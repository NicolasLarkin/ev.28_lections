...
Когда вы говорите о конкретной собаке, это у вас будет объект в программировании: объект является экземпляром класса. Это основной принцип, на котором основано 
объектно-ориентированное программирование. Так, например, моя собака Ак-Тош принадлежит к классу Dog. Его атрибутами будут name = 'Ak-Tosh'и age = '2'. У другой собаки будут другие атрибуты.
...
Это похоже на конструктор в Java. Ему требуется один обязательный параметр: self, который относится к самому объекту. Внутри метода на данный момент используется ключевое слово pass, потому что Python ожидает, что вы что-то там напечатаете. 
'''
...
Тепер у вас есть класс Dog, у которого есть имя и возраст, которые вы можете отслеживать, но он ничего не делает. Вот тут-то и появляются методы экземпляра. 
Перепишите класс, чтобы включить метод bark(). Обратите внимание, как снова используется ключевое слово def, а также аргумент self.
'''
...
'''
Теперь метод bark можно вызывать с помощью точечной нотации после создания нового объекта ak_tosh. Метод должен печатать "gav gav!" на экран. 
Обратите внимание на круглые скобки в .bark(). Они всегда используются при вызове метода. В этом случае они пусты, поскольку метод bark() не принимает никаких аргументов.
'''
