...
#     def __init__(self, name, surname, weight, nation):
...
# human2 = Human('Abu-Bakr', 'Al-Nasr', 3.8, 'Arabic')
...
# st1.read_book('Algorythms and Data structures')
# st1.read_book('Evgenyi Onegin')
# st1.do_project()
# st1.do_project()
...
# print(f'After study {st1.name}: Red :{st1.books} Learned:{st1.languages}, Total level of Knowledge {st1.knowledge}')
# print(f'Ready to work: {st1.is_ready_to_work}')
#-------------------------------------------------------------------------------------------------------------------------
# class Car():
...
# sniper2 = Sniper('Fridrih Scholtz')
...
class TriangleChecker:
    ...
    def __str__(self) -> str:
        if not all(isinstance(x,(int,float)) for x in self.sides):
            return 'нельзя построить треугольник! инвалид валью1'
        elif any(x <= 0 for x in self.sides):
            return 'нельзя посторить треугольник! инвалид валью'
        self.sides.sort()
